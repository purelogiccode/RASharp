...
public class TestHashNeo
{
    ...
    public void TestHashNeoFile()
    {
        ...
        var image = GenerateNeoFile(payloadSize, "Test Game", "TestCorp");
        var payload = TestDataGen.GenerateGenericFile(payloadSize);

        Assert.True(RcHash.GenerateFromBuffer(out var hashPayload, ConsoleIds.RcConsoleMegaDrive, payload, payloadSize));

        MockFilereader.MockFile(0, "game.neo", image, image.Length);
        Assert.True(RcHash.GenerateFromFile(out var hashFile, ConsoleIds.RcConsoleArcade, "game.neo"));
    ...
    public void TestHashNeoHeaderVariants()
    {
        ...
        const int payloadSize = 131072;
        var image1 = GenerateNeoFile(payloadSize, "Test Game", "TestCorp");
        var image2 = GenerateNeoFile(payloadSize, "test game (alt name)", "OtherTool");

        MockFilereader.MockFile(0, "game1.neo", image1, image1.Length);
        MockFilereader.MockFile(1, "game2.neo", image2, image2.Length);

        Assert.True(RcHash.GenerateFromFile(out var hash1, ConsoleIds.RcConsoleArcade, "game1.neo"));
        Assert.True(RcHash.GenerateFromFile(out var hash2, ConsoleIds.RcConsoleArcade, "game2.neo"));
    ...
    public void TestHashNeoBadMagic()
    {
        ...
        MockFilereader.MockFile(0, "game.neo", image, image.Length);
        Assert.False(RcHash.GenerateFromFile(out _, ConsoleIds.RcConsoleArcade, "game.neo"));
    }
    ...
    public void TestExtTableNeoAndSms()
    {
        ...
        Assert.NotNull(neo);
        Assert.Equal((int)ConsoleIds.RcConsoleArcade, neo.Data);

        Assert.NotNull(sms);
        Assert.Equal((int)ConsoleIds.RcConsoleMasterSystem, sms.Data);
    }
}
