// Tests for the `RetroAchievementsSharp checkkeys` subcommand (a RetroAchievementsSharp extension — not
...
/// <summary>Tests for the `RetroAchievementsSharp checkkeys` subcommand (a RetroAchievementsSharp extension — not part of the RAHasher 1.8.3 parity surface). It validates the 3DS key files (aes_keys.txt, seeddb</summary>
public class TestCheckKeys : IDisposable
{
    ...
    public void CheckKeysDefaultDirAndHelp()
    {
        ...
        Assert.Equal(0, exit);
        Assert.Contains("Usage: RetroAchievementsSharp checkkeys", stdout, StringComparison.Ordinal);
    }
}
