...
public class TestHashZip
{
    ...
    public sealed class MockZipFileDebug : MockZipFile
    {
        ...
        /// <param name="n">the number of bytes to take</param>
        /// <returns>the result</returns>
        public byte[] Take(int n)
        {
            var r = new byte[n];
            Array.Copy(Buffer, 0, r, 0, n);
            return r;
        }
        ...
        public new void AddFile(string f, uint c, uint s)
        {
            base.AddFile(f, c, s);
        }
    }
    ...
    public class MockZipFile
    {
        ...
        /// <param name="c">the c parameter</param>
        /// <returns>the result</returns>
        public int FinalizePublic(string c)
        {
            return Finalize(c);
        }

        public int Ptr;
        public readonly int[] FilePtr = new int[8];
        public int NumFiles;
        public bool IsZip64;
        ...
        public int Finalize(string comment)
        {
            ...
            var commentLen = commentBytes.Length;
            var outPos = Ptr;
            var firstCdirEntry = Ptr;
            int offset;
            ...
            for (var i = 0; i < NumFiles; i++)
            {
                ...
                /* file name */
                var filenameLen = (Buffer[inPos + 27] << 8) | Buffer[inPos + 26];
    ...
    public void TestHashArduboyFx()
    {
        ...
        var zipContents = new byte[zipSize];
        Array.Copy(zip.Buffer, 0, zipContents, 0, zipSize);
        MockFilereader.MockFile(0, "game.arduboy", zipContents, zipContents.Length);

        Assert.True(RcHash.GenerateFromFile(out var hashFile, ConsoleIds.RcConsoleArduboy, "game.arduboy"));
    ...
    public void TestHashMsdosDosz()
    {
        ...
        var zipSize = zip.Finalize("TORRENTZIPPED-FD07C52C");
        MockFilereader.InitMockFilereader(); /* xUnit runs classes in parallel; keep the global ours */
        Assert.True(zipSize <= zip.Buffer.Length);
        ...
        Assert.True(RcHash.GenerateFromFile(out var hashFile, ConsoleIds.RcConsoleMsDos, "game.dosz"));
    ...
    public void TestHashMsdosDoszWithDosc()
    {
        ...
        Assert.True(RcHash.GenerateFromFile(out var hashDosc, ConsoleIds.RcConsoleMsDos, "game.dosz"));
    ...
    public void TestHashMsdosDoszWithParent()
    {
        ...
        Assert.True(RcHash.GenerateFromFile(out var hashDosz, ConsoleIds.RcConsoleMsDos, "child.dosz"));
        ...
        Assert.True(RcHash.GenerateFromFile(out var hashDosc2, ConsoleIds.RcConsoleMsDos, "child.dosz"));
        ...
        Assert.True(RcHash.GenerateFromFile(out var hashDosc3, ConsoleIds.RcConsoleMsDos, "child.dosz"));
