// Tests for the `RetroAchievementsSharp fetch-db` subcommand (a RetroAchievementsSharp extension — not
...
/// <summary>Tests for the `RetroAchievementsSharp fetch-db` subcommand (a RetroAchievementsSharp extension — not part of the RAHasher 1.8.3 parity surface). It downloads (or copies) a RetroAchievements datab</summary>
public class TestFetchDb : IDisposable
{
    ...
    public void FetchDbHelpAndMissingSource()
    {
        ...
        Assert.Equal(0, exit);
        Assert.Contains("Usage: RetroAchievementsSharp fetch-db", stdout, StringComparison.Ordinal);

        var (exit2, stdout2, _) = RunFetchDb();
        Assert.Equal(1, exit2);
        Assert.Contains("Usage: RetroAchievementsSharp fetch-db", stdout2, StringComparison.Ordinal);
    }
}
