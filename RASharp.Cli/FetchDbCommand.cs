// New command — `RASharp fetch-db` (a RASharp extension; RAHasher 1.8.3 has
...
/// <summary>New command — `RASharp fetch-db` (a RASharp extension; RAHasher 1.8.3 has no subcommands, so the legacy positional interface and its byte-exact parity surface are u</summary>
internal static class FetchDbCommand
{
    ...
    {
        Console.WriteLine("Usage: RASharp {0} <url-or-path> [options]", Name);
        Console.WriteLine();
