...
3. **RVZ validation**: real GameCube/Wii RVZ images are converted to ISO
   with DolphinTool and hashed; the same RV files are hashed live through
   RVZSharp (`RvzFilereader`, no conversion) and the hashes must be
   identical — **6/6** (3 GameCube + 3 Wii, e.g.
   `007 - Agent Under Fire (USA)`: `11e819767dd570389fde339a301ab23e`).
   Skips with a note when the libraries, `References\DolphinTool.exe`, or
   the CLI are absent.
4. **Tier 3 — published hashes**: spot checks against the official
...
- **RVZ/WIA (GameCube/Wii)** are hashed live through RVZSharp
  (`RvzFilereader`, GPL-2.0-or-later) and validated against the
  DolphinTool-converted ISO (6/6) — a net-new capability beyond rcheevos,
  implemented and validated in-project. WUX/WUD (Wii U) have **no
  implementation in either engine**; Wii U (id 20) has no hasher at all;
  both binaries reject it with `Unsupported console for file hash: 20`. See
