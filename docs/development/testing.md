...
|---|---|---|---|
| `RASharp.Tests` | deterministic unit/vector/CLI tests — runs on every change and on every TFM | seconds | **yes** |
| `RASharp.Slow.Tests` | parity harness vs. C oracles, real-ROM libraries, RVZ/DolphinTool and published-DB spot checks | minutes | **no** (run manually) |
...
## RASharp.Tests — fast, deterministic (in the solution)
...
## RASharp.Slow.Tests — parity and real-world suites (manual)
...
```bash
dotnet build RASharp.sln                 # build the fast project first
dotnet test  RASharp.Slow.Tests          # parity harness + real-world suites
```
...
(RASharp.Cli), and requires identical hashes — proving that the
...
RASHARP_RA_DB=C:\path\to\RetroAchievements.json dotnet test RASharp.Slow.Tests --filter FullyQualifiedName~TestPublishedHashMatch
```
...
| Purpose | Command |
|---|---|
| parity only | `dotnet test RASharp.Slow.Tests --filter FullyQualifiedName~TestParity` |
| one module (fast) | `dotnet test RASharp.sln --filter FullyQualifiedName~TestHashDisc` |
| one test (fast) | `dotnet test RASharp.sln --filter "FullyQualifiedName~TestHashNes"` |
| everything fast | `dotnet test RASharp.sln` |
| everything slow | `dotnet test RASharp.Slow.Tests` |
...
(`RASharp./TestData/*.chd`), mock-built zips, synthetic 3DS keys +
...
dotnet test  RASharp.sln -c Release            # 415/415 fast
dotnet test  RASharp.sln -c Debug              # 415/415 fast
dotnet test  RASharp.Slow.Tests -c Release     # 172/172 slow (manual)
```